...
class DatabaseConfig(BaseSettings):
    ...
    # SQLite (default)
    sqlite_path: str = Field(default="data/chesserp.db", description="Ruta del archivoSQLite")

    # PostgreSQL (opcional)
    postgres_host: Optional[str] = Field(default=None, description="Host de PostgreSQL")
    postgres_port: Optional[int] = Field(default=5432, description="Puerto de PostgreSQL")
    postgres_db: Optional[str] = Field(default=None, description="Nombre de la base dedatos")
    postgres_user: Optional[str] = Field(default=None, description="Usuario dePostgreSQL")
    postgres_password: Optional[str] = Field(default=None, description="Contraseña dePostgreSQL")

    @property
    def postgres_url(self) -> Optional[str]:
        """Construye URL de PostgreSQL si está configurado"""
    ...
    def database_url(self) -> str:
        """Retorna la URL de la base de datos activa"""
        return self.postgres_url or f"sqlite:///{self.sqlite_path}"
